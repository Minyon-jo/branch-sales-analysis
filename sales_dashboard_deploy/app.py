...
def load_branches():
    """data.json 파일에서 지점 데이터를 불러온다. 파일이 없으면 기본 데이터로 초기화."""
...
def save_branches(data):
    """지점 데이터를 data.json 파일에 저장한다."""
    with open(DATA_FILE, "w", encoding="utf-8") as f:
        json.dump(data, f, ensure_ascii=False, indent=2)


# 처음 실행 시 파일에서 데이터를 로드 (앱을 껐다 켜도 유지) / 단위: 백만원
if "branches" not in st.session_state:
    st.session_state.branches = load_branches()
...
with tab1:
    ...
    if st.button("추가"):  # 콜론(:) 누락 해결
        branches.append({"지점": name, "1분기": q1, "2분기": q2, "3분기": q3})
        save_branches(branches)  # 파일에 즉시 저장
...
with tab3:
    ...
    for i in range(len(QUARTERS)):
        ...
        with cols[i]:
            ...
            # float 형태의 값을 문자열 포맷팅으로 출력하도록 수정
...
with tab4:
    ...
    st.header("등급 분포")
    dist = grade_distribution(branches)  # brnaches -> branches 오타 수정
