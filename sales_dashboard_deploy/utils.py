...
def average_sales(branch):
    """지점 평균."""
    # 지점 정보 딕셔너리에서 매출 데이터(1분기, 2분기, 3분기)의 평균을 계산해야 하므로
    # 딕셔너리의 전체 키 개수(지점명 포함)인 len(branch) 대신, 실제 분기 개수인 3으로 나누어야 합니다.
...
def quarter_average(branches, quarter):
    ...
    total = 0
    for b in branches:
        total += b[quarter]  # '1분기' 고정에서 매개변수 quarter로 수정
...
def quarter_top(branches, quarter):
    ...
    top = 0
    for b in branches:
        if b[quarter] > top:  # '<'에서 '>'로 변경하여 최댓값 검색
            top = b[quarter]
    return top
...
def rank_list(branches):
    """총매출 기준 정렬."""
    # 높은 순서(내림차순)로 정렬하기 위해 reverse=True 추가
